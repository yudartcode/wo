                                    ...
                                    <div class="row">
                                        <div class="col-6">
                                            @foreach ($data->get_first_foto as $foto)
                                                ...
                                                <img src="{{ asset('paket_wedding/' . $foto->url) }}"
                                                    ...
                                                    alt="">
                                                <?php else : ?>
                                                <img src="app-assets/images/no-image-icon.png" class="img-fluid"
                                                    alt="">
                                                <?php endif; ?>
                                            @endforeach
                                        </div>
                                        <div class="col-6">
                                            ...
                                            <div class="button-list">
                                                ...
                                                <form action="{{ route('paket-wedding.destroy', $data->id) }}"
                                                    method="post">
                                                    @csrf
                                                    @method('DELETE')
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('paket-wedding.store') }}" method="post" id="addPaketWeddingForm"
                        class="add-paket-wedding-form" enctype="multipart/form-data">
                        @csrf
    ...
    <div class="modal fade" id="editPaketWeddingModal">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('paket-wedding.update', '') }}" method="post" id="editPaketWeddingForm"
                        class="edit-paket-wedding-form" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group">
                            <label for="edit_deskripsi_paket" class="label-description">Deskripsi Paket <span
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-sm btn-danger light" data-dismiss="modal">Tutup</button>
                    <button type="submit" class="btn btn-sm btn-primary edit-paket-wedding-button"
                        id="editPaketWeddingButton">Simpan Perubahan</button>
                </div>
                </form>
            </div>
        </div>
    ...
    @section('js')
        ...
        <script>
            ...
            function detailPaketWeddingFunction(data) {
                ...
                $('#detail_harga_paket').html('Rp.' + new Intl.NumberFormat().format(data.harga_paket));
                ...
                init: function() {
                    ...
                    $("#addPaketWeddingButton").click(function() {
                        ...
                        if (myDropzone.getQueuedFiles().length > 0 && $('#addPaketWeddingForm').valid()) {
                            $('#addPaketWeddingForm').prepend(
                                '<input type="hidden" name="deskripsi_paket" value="' + CKEDITOR.instances[
                                    'description'].getData() + '">');
            ...
            Dropzone.options.editFotoPaketWedding = {
                ...
                init: function() {
                    ...
                    $('#editPaketWeddingModal').on('shown.bs.modal', function(e) {
                        ...
                        myEditDropzone.options.url = '{{ route('paket-wedding.update', '') }}' + "/" +
                            paket_wedding_id;
                        ...
                        $.ajax({
                            ...
                            success: function(result) {
                                ...
                                for (const [key, value] of Object.entries(result)) {
                                    ...
                                    myEditDropzone.emit("thumbnail", file,
                                        "{{ asset('paket_wedding') }}/" + value.url);
                    ...
                    $("#editPaketWeddingButton" + $('input[name="paket_id"]').val()).click(function() {
                        ...
                        if (checkImage.length > 0 && $('#editPaketWeddingForm' + $('input[name="paket_id"]')
                                .val()).valid()) {
                            $('#editPaketWeddingForm' + $('input[name="paket_id"]').val()).prepend(
                                '<input type="hidden" name="edit_deskripsi_paket" value="' + CKEDITOR
                                .instances['edit-description'].getData() + '">');

                            myEditDropzone.processQueue();

                            $('#editPaketWeddingForm' + $('input[name="paket_id"]').val())[0].submit();

                            $("#editPaketWeddingButton" + $('input[name="paket_id"]').val()).prop('disabled',
                                true);
                        } else {
                            $("#editPaketWeddingButton" + $('input[name="paket_id"]').val()).prop('disabled',
                                false);
                    ...
                    this.on("addedfiles", function(files) {
                        $("#editPaketWeddingButton" + $('input[name="paket_id"]').val()).click(function(e) {
                            ...
                            if (!$('#editPaketWeddingForm' + $('input[name="paket_id"]').val())
                                .valid()) {
                                $("#editPaketWeddingButton" + $('input[name="paket_id"]').val()).prop(
                                    'disabled', false);
                            } else {
                    ...
                    this.on('sendingmultiple', function(file, xhr, formData) {
                        ...
                        // var getPaketWeddingId = $('#get_paket_wedding_id' + paketId).val();
                        // Append all form inputs to the formData Dropzone will POST
                        var data = $('#editPaketWeddingForm' + $('input[name="paket_id"]').val())
                            .serializeArray();
